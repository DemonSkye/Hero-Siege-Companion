...
<template>
  <div class="run-tag-menu" @keydown.esc="emit('close')">
    ...
    <div class="run-tag-options">
      <button v-for="tag in tagOptions" :key="`${run.id}-option-${tag}`" class="run-tag-option" type="button" @click="addTag(tag)">
        #{{ tag }}
      </button>
      <button v-if="canCreate" class="run-tag-option create" type="button" @click="addTag(pending)">
        Create #{{ pending }}
      </button>
